...
MODEL_PATH = "modify it"
TOKENIZER_PATH = "modify it"
...
def eval_text(gs_file, text_file):
...
def save_csv(summary):
    summary_info = ['file','true_positive','false_positive','false_negative']
    with open('summary.csv','w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=summary_info)
        writer.writeheader()
        writer.writerows(summary)

for path in glob.glob("somesci_files/*.txt"):
    print("Analyzing file "+str(path))
    summary.append(eval_text(path.replace(".txt",".ann"),path))
